...
C!**********************************************************************
      SUBROUTINE ETRFPT(NIONIZ,ZROI,SRDATE) ! Energy TRansfer PoinTs
C!    ***************************************************************
...
C!<<<<End declarations<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<      
      ...
      IF(SRFLAG) THEN
        PRINT*, 'Subroutine ETRFPT Version: ',VDATE
        SRFLAG=.FALSE.
      END IF
      IF(DEBUG(2)) PRINT*, 'ETRFPT Input',NIONIZ
      
      DO K=1,KMAX ! Empty local histograms
        ICSITE(K)=0
      END DO ! I=1,KMAX ! Empty local histograms

      IF(DEBUG(2)) PRINT*, 'ETRFPT vor Main Loop NDIR=',NDIR
      IF(DEBUG(2)) PRINT*, 'ETRFPT vor Main Loop NIONIZ=',NIONIZ
      DO IDIR=1,NDIR ! Loop over all orientations
        IF(IDIR.GT.NDIR) GOTO 100
        IF(DEBUG(3)) PRINT*, 'ETRFPT Begin Loop IDIR',IDIR
C!      # Find target volume for all  ionizations in track      
     ...
     &                +ADJNTB(3,J,IDIR)*(XYZE(I,3))
        ...
        IF(DEBUG(3)) PRINT*, 'ETRFPT vor sort volume indices',NIONIS
...
C!      # Sort target volume indices ascending     
     ...
     &              ITARGT(I,3).LE.ITARGT(J,3))) THEN
        ...
        IF(DEBUG(3)) PRINT*, 'ETRFPT vor find unique volumes',NIONIS
C!      # Find unique target volumes and score ionizations   
 ...
 100  CONTINUE
      ...
      IF(DEBUG(2)) PRINT*, 'ETRFPT vor EXIT'
      
      END SUBROUTINE ETRFPT
