...
C!    #################################################################
      PARAMETER(VDATE='09-JUL-2022',VINPUT=210605.16d0) ! ############
C!    #################################################################
C!    09-JUL-2022 HR: 
C!      - Adapted for use with Linux file names
C!    04-JUL-2022 HR: 
...
C!                   (7) GEANT4 code for interaction type
C!                   (8) is there for future use
      CHARACTER CODE*8
      INTEGER*2 IDT(8),NDT
...
C!        Init counters       
          ...
          ENDDO
          IF (NFORMT.GT.0) THEN 
*            FILOUT=FILENM(1:NFORMT)//PREFIX//FILENM(NFORMT+1:80)
            FILOUT=PREFIX//FILENM(NFORMT+1:80)
          ELSE
          ...
          DO I=1,4
            XYZE(NTRANS,I)=RLINE(IDT(I))
          END DO
          ITYPE(NTRANS)=IONIZE(RLINE(IDT(7)))
          NTRANS=NTRANS+1
  ...
  100     CONTINUE ! >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
          ...
          ELSE ! Entry from a new track was read
            NTRANS=NTRANS-1 ! Decrease to number of interactions in track
            IF(DEBUG(1)) PRINT*, 'Before CALL CLUSTR'
            IF(MOD(NTRACS,10).EQ.0) PRINT*, NTRACS
            DO I=1,NTRANS
              DO J=1,3
                XYZE(I,J)=XYZE(I,J)*1000. ! Ziad used micrometers
              END DO
            END DO
            CALL CLUSTR(NTRANS,ZROI,SRDATE) 
            VDATES(2)=SRDATE
            IF(NTRACS.EQ.1) THEN
              OPEN(LUNT,FILE=FILOUT,STATUS='UNKNOWN') 
              WRITE(LUNT,'(a)') 'Output from IC_3E.f - Version '//
     ...
     &            ' in ion tracks for DSITE=',DSITE,' nm '   
              ...
              WRITE(LUNT,'(a)') 'Track x/nm y/nm z/nm E_imp/eV  ICS  '   
            END IF            
            DO K=1,NSITES-1 
              DO I=K+1,NSITES
            ...
            END DO !  K=1,NSITES-1             
            DO K=1,NSITES 
             CALL WRLINE(LUNT,IT,XYZE(K,1),XYZE(K,2),XYZE(K,3),
     &                   XYZE(K,4),ICSIZE(K))
            END DO ! K=1,NSITES
...
C!______________________________________________________________________      
      ...
      INCLUDE 'BLINIT.f'       ! <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
      INCLUDE 'IC_3E_SUBS.f'   ! <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
      INCLUDE 'RFINIT.f'
      INCLUDE 'TSTAMP.f'

C!______________________________________________________________________
      INTEGER*1 FUNCTION IONIZE(VAL)
      INTEGER*4 IVAL
      REAL*8 VAL
      IVAL=NINT(VAL) 
      IONIZE=0
      IF((IVAL.EQ.2).OR.(IVAL.GE.4)) IONIZE=1
      RETURN
      END
C!______________________________________________________________________
      SUBROUTINE WRLINE(LUN,IT,X,Y,Z,EIMP,ICS)
      ...
      WRITE(LUN,FORMTS) IT,X,Y,Z,EIMP,ICS
...
C!______________________________________________________________________
      ...
      IF(DF.EQ.0.0) THEN
        LFLOAT=4
      ELSE
        LFLOAT=5+INT(LOG10(ABS(DF)))
        IF(LFLOAT.LT.5) LFLOAT=5
