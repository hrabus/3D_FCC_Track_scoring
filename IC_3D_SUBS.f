...
C!**********************************************************************
      SUBROUTINE CLUSTR(NIONIZ,ZROI,SRDATE) 
C!    ***************************************************************
C!    Determine targets with ionization clusters
C!    ***************************************************************
C!>>> Declarations >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
      IMPLICIT NONE
...
C!    04-JUL-2022 HR: Changed the year of the first three modifications
...
C!    ----- Parameters for track and radial distance histograms
      INTEGER*4 KMAX, MIONIZ, MXRPOS, MXYPOS, MXTARG
      PARAMETER(KMAX=4, MIONIZ=100000, MXRPOS=41, MXTARG=1001,
     &          MXYPOS=4*MXRPOS*(MXRPOS-1)+1)
C!    ----- Local scalars
      INTEGER*4 I, IDIR, IR, J, K
      INTEGER*4 IHOLD, NIONIS
      REAL*8 SAVXYZ, SPROD
C!    ----- Local arrays
      INTEGER*4 ITARGT(MIONIZ,3)
...
C!<<<<End declarations<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<      
      ...
      IF(SRFLAG) THEN
        ...
        SRFLAG=.FALSE.
      END IF
      IF(DEBUG(2)) PRINT*, 'CLUSTR Input',NIONIZ
      ...
      IF(DEBUG(2)) PRINT*, 'CLUSTR vor Main Loop NIONIZ=',NIONIZ
...
C!      # Find target volume for all  ionizations in track      
        NIONIS=0
        DO I=1,NIONIZ 
          IF(XYZ(I,3).GE.ZROI(1).AND.XYZ(I,3).LE.ZROI(2)) THEN
            NIONIS=NIONIS+1
            DO J=1,3
              SPROD= ADJNTB(1,J,IDIR)*XYZ(I,1)
     &                +ADJNTB(2,J,IDIR)*XYZ(I,2)
     &                +ADJNTB(3,J,IDIR)*(XYZ(I,3))
              ITARGT(NIONIS,J)=NINT(SPROD)
            END DO ! J=1,3
          END IF
        END DO ! I=1,NIONIZ
      
        IF(DEBUG(3)) PRINT*, 'CLUSTR vor sort volume indices',NIONIS
...
C!      # Sort target volume indices ascending     
     ...
     &              ITARGT(I,3).LE.ITARGT(J,3))) THEN
              DO IR=1,3 
                ...
                ITARGT(J,IR)=IHOLD
                SAVXYZ=XYZ(I,IR)     ! 14-MAR-2021
                XYZ(I,IR)=XYZ(J,IR)  ! 14-MAR-2021
                XYZ(J,IR)=SAVXYZ     ! 14-MAR-2021
              END DO ! IR=1,3
            END IF
          END DO ! J=1,NIONIS
        END DO ! I=1,NIONIS
        
        IF(DEBUG(3)) PRINT*, 'CLUSTR vor find unique volumes',NIONIS
