...
C!**********************************************************************
      SUBROUTINE TARG3D(NIONIZ,NRPOS,NTARG,NXYPOS,SRDATE) 
C!    ***************************************************************
...
C!    04-JUL-2022 HR: Changed the year of the first five modifications
C!                    in the history back to the correct value (2020 or
C!                    MMXX in roman notation. The wrong dates have been
C!                    due to an automatic replacement in previous edits.
...
C!    02-APR-2000 HR: Included scoring of correlations
C!    28-MAR-2020 HR: Cleaned up unused variables
C!    15-MAR-2020 HR:
C!      - Added VDATE and modified COMMON BLOCK VERBOSE and added output of VDATE on first call
C!      - Added input variable NXYPOS to fix bug in processing off-axis positions
C!      - Fixed bug: Case that no ionizations occur at all was not properly handled.
C!    14-MAR-2020 HR:
C!      - Added output of cluster positions for analysis of cluster of clusters
...
C!    ----- Parameters for track and radial distance histograms
     ...
     &          MXTRG2=513, MXYPOS=4*MXRPOS*(MXRPOS-1)+1)
C!    ----- Local scalars
      INTEGER*4 I, IDIR, IDIST, IPOS, IR, J, K
      INTEGER*4 IHOLD, NCOUNT, NIONIS
      REAL*8 WEIGHT, SAVXYZ, SPROD
C!    ----- Local arrays
...
C!<<<<End declarations<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<      
      ...
      IF(SRFLAG) THEN
        PRINT*, 'Subroutine TARG3D Version: ',VDATE
        SRFLAG=.FALSE.
      END IF
      IF(DEBUG(2)) PRINT*, 'TARG3D Input',NIONIZ,NRPOS,NTARG,NXYPOS
...
C!    End init local histograms
      ...
      IF(DEBUG(2)) PRINT*, 'TARG3D vor Main Loop NDIR=',NDIR
      IF(DEBUG(2)) PRINT*, 'TARG3D vor Main Loop NIONIZ=',NIONIZ
      DO IDIR=1,NDIR ! Loop over all orientations
        IF(IDIR.GT.NDIR) GOTO 100
        IF(DEBUG(3)) PRINT*, 'TARG3D Begin Loop IDIR',IDIR
C!      # Find target volume for all  ionizations in track      
     ...
     &                +ADJNTB(3,J,IDIR)*(XYZ(I,3)-ZROI(2))
        ...
        IF(DEBUG(3)) PRINT*, 'TARG3D vor sort volume indices',NIONIS
...
C!      # Sort target volume indices ascending     
     ...
     &              ITARGT(I,3).LE.ITARGT(J,3))) THEN
        ...
        IF(DEBUG(3)) PRINT*, 'TARG3D vor find unique volumes',NIONIS
C!      # Find unique target volumes and score ionizations   
     ...
     &       .OR.ITARGT(I,3).NE.ITARGT(I-1,3)) THEN
        ...
        IF(DEBUG(3)) PRINT*, 'TARG3D Begin Loop IPOS',NXYPOS
        DO IPOS=1,NXYPOS ! Loop over all track positions ! 15-MAR-2020
...
*     &                +ADJNTB(3,J,IDIR)*ZROI(2)
          ...
          IF(DEBUG(4)) PRINT*, 'TARG3D vor Score hit targets',ICROI,
     &                      IPOS, XROI(IPOS)
C!        # Score hit targets
     ...
     &                     *(ITARGT(I,K)-ICROI(K))
            ...
            END DO
            NCOUNT=0
            IF(ABS(IDIST).LE.NRROI2) THEN ! count if inside ROI
...
*              IF(DEBUG(5)) PRINT*, 'Count hit targets',IDIST,NCOUNT,IPOS
          ...
          IF(DEBUG(3)) PRINT*, 'TARG3D vor add to sum arrays',NDIR,IPOS
C!      # Add this histogram to sum arrays
          IR=IRAD(IPOS)
          IF(DEBUG(3)) PRINT*, 'TARG3D vor add to sum arrays',IR,ICSITE
...
C!        End 02-APR-2021 <<<<<<<<<<
          ...
          IF(DEBUG(3)) PRINT*, 'TARG3D nach sum arrays',NDIR,IDIR
...
C!        End 02-APR-2021 <<<<<<<<<<
      ...
      IF(DEBUG(2)) PRINT*, 'TARG3D vor EXIT'
